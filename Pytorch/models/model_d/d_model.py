...
class Weighted_MSE(nn.Module):
    ...
    def forward(self, input, label, weighted_matrix):
        out = (input - label)**2
        out = out * weighted_matrix.expand_as(label)
        loss = out.mean()
        return loss
...
class NET(nn.Module):
    ...
    def __init__(self, len_seq, batch_size, hidden_dimension, num_layers):
        ...
        self.layer1 = nn.Sequential(nn.Conv2d(4, 24, kernel_size=(5, 5), padding=5, stride=(2, 2)), nn.ELU())
        ...
        self.LSTM = nn.LSTM(input_size=self.hidden_dimension, hidden_size=self.hidden_dimension, num_layers=self.num_layers, batch_first=True, dropout=0.3)
