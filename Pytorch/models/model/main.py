...
from utils.read_csv         import load_data_sequence_nodepth_2
import torch
import torch.nn             as nn
from models.model.model            import NET, Weighted_MSE
from models.model.train            import train
from models.model.test             import test
...
def load_Dataset(len_sequence, train_path=None, valid_path=None, test_path=None):
    if test_path == None:
        ...
        imm_train, train_coordinates,  _ = load_data_sequence_nodepth_2(train_path, len_sequence, 'train')
        print()
        print("Load Validation Set")
        imm_valid, valid_coordinates,  _ = load_data_sequence_nodepth_2(valid_path, len_sequence, 'validation')
        ...
        print("Load Test Set")
        imm_test, test_coordinates, image_path = load_data_sequence_nodepth_2(test_path, len_sequence, 'test')
...
def main():
    ...
    if args.test == None:
        ...
        if (args.valid == None or args.weight == None):
            ...
            save_weight_path = args.weight + 'weight_' + str(epoch) + '_lenseq_' + str(args.sequence) + '_' + str(timestamp)
    ...
    if args.train == None:
        if (args.model == None):
            ...
            test(model, criterion, args.model, batch_size, test_loader, image_path, dev, len_seq, loss)
